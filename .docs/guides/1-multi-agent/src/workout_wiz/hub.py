...
_ROUTER_MODEL = os.environ.get("ROUTER_MODEL", "claude-3-haiku-20240307")
...
def _coach_stub(state: AgentState) -> dict[str, Any]:
    """Stub coach sub-agent."""
    t0 = time.monotonic()
    response = AIMessage(content="[coach stub]")
    latency_ms = int((time.monotonic() - t0) * 1000)
    audit_entry = {
        "event": "coach",
        "model": _ROUTER_MODEL,
...
def _workout_gen_stub(state: AgentState) -> dict[str, Any]:
    """Stub workout generator sub-agent."""
    t0 = time.monotonic()
    response = AIMessage(content="[workout_gen stub]")
    latency_ms = int((time.monotonic() - t0) * 1000)
    audit_entry = {
        "event": "workout_gen",
        "model": _ROUTER_MODEL,
...
def _workout_log_stub(state: AgentState) -> dict[str, Any]:
    """Stub workout logger sub-agent."""
    t0 = time.monotonic()
    response = AIMessage(content="[workout_log stub]")
    latency_ms = int((time.monotonic() - t0) * 1000)
    audit_entry = {
        "event": "workout_log",
        "model": _ROUTER_MODEL,
