import uuid
from sqlalchemy import String, Boolean, Integer, Text, Float, text
...
class Exercise(Base):
    ...
    movement_patterns: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False, server_default=text("'{}'::text[]"))
    ...
    joints_loaded: Mapped[list[str]] = mapped_column(ARRAY(String), nullable=False, server_default=text("'{}'::text[]"))
    side: Mapped[str | None] = mapped_column(String(10), nullable=True)
    estimated_rep_duration: Mapped[float | None] = mapped_column(Float, nullable=True)
